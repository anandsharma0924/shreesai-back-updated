...
const BASE_URL = process.env.UPLOAD_PATH;
...
exports.addProperty = [
  ...
  async (req, res) => {
    try {
      ...
      const validPropertyTypes = ["sell", "rent"];
      ...
      const images = image.map((filename) => `${BASE_URL}/uploads/${filename}`);
      const videoUrl = video ? `${BASE_URL}/uploads/${video}` : null;
